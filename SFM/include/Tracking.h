...
class Tracking {
 public:
  Tracking(System* system, ORBVocabulary* vocabulary, Map* map,
           ...
           const std::string& string_setting_file);
 ...
 public:
  ...
  std::list<Eigen::Matrix4d> relative_frame_poses_;
  std::list<KeyFrame*> reference_keyframes_;
  std::list<double> frame_times_;
  std::list<bool> do_lostes_;
...
//  protected:
  ...
  void SaveKeyFrame(KeyFrame*, cv::Mat&);
