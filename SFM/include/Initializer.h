...
class Initializer {
 ...
 public:
  ...
  // motion
  bool Initialize(const Frame& current_frame, const std::vector<int>& matches,
                  Eigen::Matrix3d& R21, Eigen::Vector3d& t21,
                  std::vector<Eigen::Vector3d>& vP3D, std::vector<bool>& is_triangulated);
 ...
 private:
  ...
  Eigen::Matrix3d ComputeH21(const std::vector<cv::Point2f>& vP1,
                             const std::vector<cv::Point2f>& vP2);
  Eigen::Matrix3d ComputeF21(const std::vector<cv::Point2f>& vP1,
                             const std::vector<cv::Point2f>& vP2);
  ...
  bool ReconstructF(std::vector<bool>& vbMatchesInliers, Eigen::Matrix3d& F21,
                    Eigen::Matrix3d& K, Eigen::Matrix3d& R21,
                    Eigen::Vector3d& t21, std::vector<Eigen::Vector3d>& vP3D,
                    std::vector<bool>& is_triangulated, float minParallax,
                    int minTriangulated);

  bool ReconstructH(std::vector<bool>& vbMatchesInliers, Eigen::Matrix3d& H21,
                    Eigen::Matrix3d& K, Eigen::Matrix3d& R21,
                    Eigen::Vector3d& t21, std::vector<Eigen::Vector3d>& vP3D,
                    std::vector<bool>& is_triangulated, float minParallax,
                    int minTriangulated);
  ...
  void Normalize(const std::vector<cv::KeyPoint>& vKeys,
                 std::vector<cv::Point2f>& vNormalizedPoints, Eigen::Matrix3d& T);
  ...
  int CheckRT(const Eigen::Matrix3d& R, const Eigen::Vector3d& t,
              ...
              const Eigen::Matrix3d& K, std::vector<Eigen::Vector3d>& vP3D,
