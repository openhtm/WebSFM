...
void KeyFrame::ComputeBoW() {
  if (bow_vector_.empty() || feature_vector_.empty()) {
    ...
    orb_vocabulary_->transform(descriptors_, 4, bow_vector_, feature_vector_);
...
void KeyFrame::UpdateConnections() {
  ...
  {
    std::lock_guard<std::mutex> lockMPs(mutex_features_);
...
void KeyFrame::SetBadFlag() {
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_connections_);
    std::lock_guard<std::mutex> lock1(mutex_features_);
...
std::vector<size_t> KeyFrame::GetFeaturesInArea(const float& x, const float& y,
  ...
  const int min_cell_x =
      std::max(0, (int)floor((x - min_x_ - r) * grid_element_width_inv_));
  if (min_cell_x >= grid_cols_) {
    return indices;
  }

  const int max_cell_x =
      std::min((int)grid_cols_ - 1,
  ...
  const int min_cell_y =
      std::max(0, (int)floor((y - min_y_ - r) * grid_element_height_inv_));
  if (min_cell_y >= grid_rows_) {
    return indices;
  }

  const int max_cell_y =
      std::min((int)grid_rows_ - 1,
