...
void LoopClosing::InsertKeyFrame(KeyFrame* keyframe) {
  std::lock_guard<std::mutex> lock(mutex_loop_queue_);
  if (keyframe->id_ != 0) {
...
bool LoopClosing::DetectLoop() {
  ...
  const fbow::fBow& current_bow_vector = current_keyframe_->bow_vector_;
  float minScore = 1;
  for (size_t i = 0; i < connected_keyframes.size(); i++) {
    KeyFrame* keyframe = connected_keyframes[i];
    if (keyframe->isBad()) continue;
    const fbow::fBow& bow_vector = keyframe->bow_vector_;

    float score = fbow::fBow::score(bow_vector, current_bow_vector);
...
void LoopClosing::CorrectLoop() {
  ...
  if (isRunningGBA()) {
    std::lock_guard<std::mutex> lock(mutex_global_BA_);
  ...
  is_stop_global_BA_ = false;
  thread_global_BA_ = new std::thread(&LoopClosing::RunGlobalBundleAdjustment, this,
...
void LoopClosing::RequestReset() {
  ...
  while (true) {
    {
      std::lock_guard<std::mutex> lock2(mutex_reset_);
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF) {
  spdlog::info("Starting Global Bundle Adjustment");
  ...
  // tree
  {
    ...
    if (!is_stop_global_BA_) {
      spdlog::info("Global BA finished, updating map");
      ...
      std::list<KeyFrame*> keyframes_to_check(map_->keyframe_origins_.begin(),
