...
Tracking::Tracking(System* system, ORBVocabulary* vocabulary, Map* map,
    ...
    : state_(NO_IMAGES_YET),
      ...
      keyframe_dir_(system->keyframe_dir_)
{
  cv::FileStorage fSettings(string_setting_file, cv::FileStorage::READ);
  // set imsize
  int imwidth = fSettings["Camera.width"];
  int imheight = fSettings["Camera.height"];
  imsize_ = cv::Size(imwidth, imheight);
  ...
  spdlog::info("Camera Parameters:");
  spdlog::info("- fx: {}, fy: {}, cx: {}, cy: {}", fx, fy, cx, cy);
  spdlog::info("- k1: {}, k2: {}, p1: {}, p2: {}, k3: {}", DistCoef.at<float>(0),
               DistCoef.at<float>(1), DistCoef.at<float>(2),
               DistCoef.at<float>(3), k3);
  spdlog::info("- fps: {}", fps);
  ...
  spdlog::info("- Number of Features: {}", n_features);
  spdlog::info("- Scale Levels: {}", n_levels);
...
Eigen::Matrix4d Tracking::GrabImageMonocular(const cv::Mat& img,
  ...
  if(img.channels() != 1 && img.channels() != 3) {
    spdlog::critical("Tracking image should be grayscale or RGB format");
  }
  ...
  if (img.channels() == 3) 
    cv::cvtColor(img, img_gray_, CV_RGB2GRAY);
...
void Tracking::Track() {
  ...
  std::lock_guard<std::mutex> lock(map_->mutex_map_update_);
  ...
  if (state_ == NOT_INITIALIZED) {
    ...
    if (state_ == LOST) {
      if (map_->KeyFramesInMap() <= 5) {
        spdlog::warn("Track lost soon after initialisation, reseting...");
        system_->Reset();
        return;
      }
    }
...
void Tracking::CreateInitialMapMonocular() {
  ...
  KeyFrame* current_keyframe = new KeyFrame(current_frame_, map_, keyframe_database_);
  ...
  SaveKeyFrame(init_keyframe, init_img_rgb_);
  map_->AddKeyFrame(current_keyframe);
  SaveKeyFrame(current_keyframe, img_rgb_);
  ...
  if (median_depth < 0 || current_keyframe->TrackedMapPoints(1) < 80) {
    spdlog::warn("Wrong initialization, reseting...");
    Reset();
    return;
  }
...
void Tracking::CreateNewKeyFrame() {
  ...
  SaveKeyFrame(keyframe, img_rgb_);
...
void Tracking::SaveKeyFrame(ORB_SLAM2::KeyFrame* kf, cv::Mat& img) {
  if(keyframe_dir_.size() == 0)
    return;
  
  unsigned long id = kf->id_;
  std::string path = keyframe_dir_ + std::to_string(id) + ".png";
  cv::imwrite(path, img);
}
