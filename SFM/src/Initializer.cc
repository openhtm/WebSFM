...
bool Initializer::Initialize(const Frame& current_frame,
  ...
  for (size_t i = 0, iend = matches.size(); i < iend; i++) {
    if (matches[i] >= 0) {
      index_matches_.push_back(std::make_pair(i, matches[i]));
  ...
  // got current millon sec
  auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch());
  double timestamp = (double)ms.count();
  ...
  for (int it = 0; it < max_iterations_; it++) {
    ...
    for (size_t j = 0; j < 8; j++) {
      int d = vAvailableIndices.size();
      int randi = int(((double)rand() / ((double)INT_MAX + 1.0)) * d);
  ...
  std::thread threadH(&Initializer::FindHomography, this, ref(vbMatchesInliersH),
                 std::ref(SH), std::ref(H));
  std::thread threadF(&Initializer::FindFundamental, this, ref(vbMatchesInliersF),
...
int Initializer::CheckRT(const Eigen::Matrix3d& R, const Eigen::Vector3d& t,
  ...
  if (nGood > 0) {
    ...
    size_t index = std::min(50, int(vCosParallax.size() - 1));
