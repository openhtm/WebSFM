...
int ORBmatcher::SearchByBoW(KeyFrame* pKF, Frame& F,
  ...
  const auto& vFeatVecKF = pKF->feature_vector_;
  ...
  auto Fit = F.feature_vector_.begin();
  auto KFend = vFeatVecKF.end();
  auto Fend = F.feature_vector_.end();
