...
void CeresOptimizer::BundleAdjustment(const std::vector<KeyFrame*>& keyframes,
  ...
  if (keyframes.empty()) {
    ...
    spdlog::info("No keyframes, not to optimize");
    return;
  }
...
int CeresOptimizer::PoseOptimization(Frame* frame) {
  ...
  int n_bad = 0;
  {
    std::lock_guard<std::mutex> lock(MapPoint::global_mutex_);
    // Get frame Pose
...
reoptimize:
  ...
  std::lock_guard<std::mutex> lock(map->mutex_map_update_);
  if (!to_erase.empty()) {
...
int CeresOptimizer::OptimizeSim3(KeyFrame* keyframe_1, KeyFrame* keyframe_2,
  ...
  spdlog::info("n_correspondences: {} n_bad: {}", n_correspondences, n_bad);
...
void CeresOptimizer::OptimizeEssentialGraph(
  ...
  for (size_t i = 0; i < all_keyframes.size(); i++) {
    ...
    for (auto& it : connected_keyframes) {
      ...
      if (keyframe_n && keyframe_n != parent_keyframe &&
        ...
        if (!keyframe_n->isBad() && keyframe_n->id_ < keyframe->id_) {
          if (inserted_edges.count(
                  std::make_pair(std::min(keyframe->id_, keyframe_n->id_),
                                 std::max(keyframe->id_, keyframe_n->id_))))
            continue;
  ...
  ceres::Solve(options, &problem, &summary);
  spdlog::info("OptimizeEssentialGraph: {}", summary.BriefReport());
