...
void LocalMapping::CreateNewMapPoints() {
  ...
  for (size_t i = 0; i < neighbor_keyframes.size(); i++) {
    ...
    vector<std::pair<size_t, size_t> > matched_indices_;
...
void LocalMapping::RequestStop() {
  ...
  std::lock_guard<std::mutex> lock2(mutex_new_keyframes_);
  is_abort_BA_ = true;
}
...
void LocalMapping::Release() {
  std::lock_guard<std::mutex> lock(mutex_stop_);
  std::lock_guard<std::mutex> lock2(mutex_finish_);
  if (is_finished_) {
    return;
  }
  is_stopped_ = false;
  ...
  spdlog::info("Local Mapping RELEASE");
}
...
void LocalMapping::SetFinish() {
  std::lock_guard<std::mutex> lock(mutex_finish_);
  is_finished_ = true;
  std::lock_guard<std::mutex> lock2(mutex_stop_);
  is_stopped_ = true;
}
